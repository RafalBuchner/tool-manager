from vanilla import HUDFloatingWindow, List, ImageListCell, CheckBoxListCell, GradientButton, Group, PopUpButtonListCell, CheckBox, SquareButton  # type: ignore
from mojo.events import installTool, uninstallTool, getToolOrder, setToolOrder, setActiveEventTool, addObserver, removeObserver, extractNSEvent  # type: ignore
from mojo.UI import AllGlyphWindows, GetFile, PutFile, AllGlyphWindows  # type: ignore
from mojo.extensions import getExtensionDefault, setExtensionDefault  # type: ignore
from plistlib import load, dump
from AppKit import NSImageNameActionTemplate  # type: ignore
from CustomAppKit import VerticallyCenteredTextFieldCell
from defconAppKit.windows.baseWindow import BaseWindowController  # type: ignore
from mojo.roboFont import OpenWindow  # type: ignore
from lib.eventTools.eventManager import EventManager  # type: ignore
...
class ToolManagerWindow(BaseWindowController):
    modifiers = ["no-modifiers", "command", "option", "control"]
    ...
    def __init__(self):
        ...
        if settings is None:
            ...
            self.hideToolbar = settings.get("hideToolbar", False)
        if settings is None:
            self.openWithGlyphWindow = False
        else:
            self.openWithGlyphWindow = settings.get("openWithGlyphWindow", False)
    ...
    def initUI(self):
        ...
        self.w = HUDFloatingWindow(
            (0, 0, w, h), self.windowTitle, minSize=self.minSize, autosaveName=key
        )
        ...
        columnInfo = [
            dict(
                title="icon",
                cell=ImageListCell(),
                width=self.rowHeight + self.rowHeight / 8,
            ),
            dict(
                title="tool",
                cell=VerticallyCenteredTextFieldCell("mini"),
                editable=False,
            ),
        ...
        self.w.palette.list = List(
            ...
            drawFocusRing=True,
            editCallback=self.listChangedCallback,
        )  # ,dragSettings=dict(type=toolOrderDragType, callback=self.dragCallback), selfDropSettings=dict(type=toolOrderDragType, operation=NSDragOperationMove, callback=self.dropListSelfCallback))
        self.selectionCallback(self.w.palette.list)
        self.w.palette.openSettings = GradientButton(
            (5, -66 + 5, -5, -5),
            imageNamed=NSImageNameActionTemplate,
            sizeStyle="mini",
            callback=self.openSettingsCallback,
        )
        ...
        columnInfo = [
            ...
            dict(
                title="modifier",
                cell=PopUpButtonListCell(self.modifiers),
                binding="selectedValue",
            ),
        ]
        self.w.settings.list = List(
            (5, 0, -0, -66),
            [],
            columnDescriptions=columnInfo,
            rowHeight=self.rowHeight,
            ...
            drawFocusRing=True,
            editCallback=self.hotkeyEditCallback,
        )
        self.w.settings.hideToolbar = CheckBox(
            (5, -66 + 5, -5, 15),
            "hide toolbar",
            sizeStyle="mini",
            callback=self.hideToolbarCallback,
            value=self.hideToolbar,
        )
        ...
        self.w.settings.showOnLaunchChB = CheckBox(
            (5, -44 + 5, -5, 15),
            "show on launch",
            sizeStyle="mini",
            callback=self.showOnLaunchCallback,
            value=self.showOnLaunch,
        )
        self.w.settings.exportBtn = SquareButton(
            (5, -22 + 2, self.settingsWidth / 2 - 5 - 2.5, 15),
            "export prefs",
            sizeStyle="mini",
            callback=self.exportImportCallback,
        )
        self.w.settings.importBtn = SquareButton(
            (self.settingsWidth / 2 + 2.5, -22 + 2, -5, 15),
            "import prefs",
            sizeStyle="mini",
            callback=self.exportImportCallback,
        )
    ...
    def windowClose(self, info):
        ...
        hideToolbar = False
        if self.w.settings.hideToolbar.get() == 1:
            hideToolbar = True
        openWithGlyphWindow = False
        setExtensionDefault(
            settingskey,
            dict(hideToolbar=hideToolbar, openWithGlyphWindow=openWithGlyphWindow),
        )
    ...
    def customShortcutCallback(self, info):
        ...
        for toolName in self.hotkeys:
            ...
            if len(hotkey.split(" ")) == 2:
                ...
                if (
                    event["keyDownWithoutModifiers"] == char
                    and event[modifierDict[modifier]] != 0
                ):
                    setActiveEventTool(toolName)
                    break
            else:
            ...
            if newValue != oldValue:
                ...
                if not newValue:
                    if index in selection:
                        ...
                        listOfValues = [
                            self.toolDescription[name] for name in self.toolNames
                        ]
                        newSelection = []
